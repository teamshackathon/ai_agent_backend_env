...
# 必要なパッケージのインストール
RUN apt-get update && \
    apt-get install -y --no-install-recommends gcc default-libmysqlclient-dev pkg-config build-essential libpq-dev && \
...
# ロケールの設定
RUN apt-get update && \
    apt-get install -y --no-install-recommends locales libmariadb3 libmariadb-dev curl libpq-dev && \
